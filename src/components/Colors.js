...
const Colors = () => {
	...
	const createColors = () => {
		...
		for (let i = 0; i < colors.length; i++) {
			...
			const colorClass = color.visible ? 'color-cell' : 'color-cell transparent-cell';

			cells.push(<div className={colorClass} onClick={() =>
				changeColor(color)} key={i} style={{ backgroundColor: colors[i].color }}></div>);
		}
